...
void Solucao::contruirFal() {
	...
	for (int j = 1; j < d->j; j++) {
		...
		for (int posi = 0; posi < possiveis_qi[j].size(); posi++) {
			int cadi = possiveis_qi[j][posi];
			if (Ti[cadi] + D[cadi] == ti_j) {
				fal[j] = cadi;
				x[j] = posi;
				preencheu = true;
				break;
		...
		if (!preencheu) { // se nao preencheu ainda entao deve pegar o ultimo predecessor
			preencherFalXComAntecessores(j);
		}

	}

	/*for (int j = 1; j < d->j; j++) {
	 vector<int>::iterator sx =  possiveis_qi[j].begin();
	 for(;sx != possiveis_qi[j].end();sx++){
	 cout << *sx << " ";
	 }
	 cout << endl << j << " fal:" << fal[j] << " X: " << x[j] << endl;
	 }*/
}

/*busca no tempo das antecessoras quem deve ser o fal, e preenche tambem o x*/
void Solucao::preencherFalXComAntecessores(int j) {
	int ult_pre = 0;
	int ult_tf_pre = 0;
...
void Solucao::preencherPossiveisQi() {	//ok
...
void Solucao::preencherPossiveisQiComPredecessores() {//preenche com os predecessores
	// sucessores direto e indireto, que não pode iniciar depois de pos
	for (int at = 0; at < d->j; at++) {
		list<int>::iterator pre = d->H[at].begin();
		for (; pre != d->H[at].end(); pre++) {
			possiveis_qi[at].push_back(*pre);
		}

		/*cout<< at <<": ";
		vector<int>::iterator pos = possiveis_qi[at].begin();
		for (; pos != possiveis_qi[at].end(); pos++) {
			cout << *pos<< " ";
		}
		cout << endl;*/
	}
