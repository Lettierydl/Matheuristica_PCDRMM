...
public:
	...
	void iniciarEnxame(int tamanhoPopulacao, int maxVparticula,
			int minVpartucula, vector<Solucao*> *enxame);
	void addFronteiraDeParetoPSO(vector<Solucao*> *enxame,
			list<Solucao*> *gbests);//metodo que analiza solucao, e adiciona a variavel gbests com a ideia da fronteira de pareto
	void atualizarPosicoes(int tamanhoPopulacao, vector<Solucao*>* enxame);
	void atualizarVelocidades(int tamanhoPopulacao, vector<Solucao*>* enxame,
			float w, float c1, float c2, int maxVparticula, int minVpartucula);
	...
	Solucao* construicaoGulosa(float alfa);
	/*list<Candidato> criarPossiveisCandidatos(list<int> prox_candidatos);
	list<Candidato> criarListaRestritaDeCandidatos(list<Candidato> lc, float alfa);
	*/
	...
	Solucao* vizinhancaCPLEX(Solucao *s);
