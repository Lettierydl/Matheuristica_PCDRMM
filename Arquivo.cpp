...
void Arquivo::instancia_denise(const string& nomeArquivo) {
	...
	this->funcao.open((nomeArquivo.substr(nomeArquivo.find(".")) + ".tx2").c_str(), ios::in);
	if (!(this->dados.is_open())) {
		cout << "arquivo nao pode ser aberto" << endl;
	}
}
...
void Arquivo::instancia_PSPLIB(const string& nomeArquivo) {
	...
	if (!(this->dados.is_open())) {
		cerr << "arquivo "<< nomeArquivo << " nao pode ser aberto" << endl;
	}else{
		//cerr << "arquivo "<< nomeArquivo << " Aberto..." << endl;
	}
}
