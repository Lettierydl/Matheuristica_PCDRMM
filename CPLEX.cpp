...
Solucao* CPLEX::construirModosSolucaoParcial(Solucao * s) {
	try {
		...
		int T = s->tempo + 1; // prazo para o makespan
...
Solucao * CPLEX::construirSolucaoParcial(Solucao * s,
	...
	try {
		...
		list<int>::iterator it = atividades_fixas.begin();
		for (; it != atividades_fixas.end(); it++) {
			int tf = s->Ti[*it] + s->D[*it];
			int mi = s->M[*it];
			//	cout << *it << "\t" << mi << "\t" << tf << endl;
			modelo.add(x[*it][mi][tf] == 1);
		}
...
void CPLEX::restricaoCusto(int T, IloEnv env,
		IloArray<IloArray<IloBoolVarArray> > x, const IloModel& modelo,
		const IloNumVarArray& a, Dados* d) {
	//mostrar que o recurso é renovavel
	//terceira restricao
	//garante que durante a execucao da atividade a quantidade ak nao será exedida
...
void CPLEX::restricaoJanelaDeTempoDeJ(IloEnv env,
		IloArray<IloArray<IloBoolVarArray> > x, const IloModel& modelo,
		Dados* d, int T) {
...
void CPLEX::restricaoDePrecedencia(IloEnv env, int T,
		IloArray<IloArray<IloBoolVarArray> > x, const IloModel& modelo,
		Dados* d) {
//segunda restricao
//garante que as restricoes de precedencias sao obedecidas
...
Solucao* CPLEX::OptimizeCplex(Dados *d) {
